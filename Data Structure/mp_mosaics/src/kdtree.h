...
class KDTree
{
  ...
  public:
    ...
    KDTreeNode* build(vector<Point<Dim>>& points_, int d, unsigned int left, unsigned int right);
    ...
    void findHelper(KDTreeNode* node, const Point<Dim>& query, Point<Dim>& best, int curDim) const;
